...
- **Productos** — al entrar ves **una carpeta por categoría** (aires, refrigeradores, congeladores, estufas y vitrinas). Abres la carpeta de la categoría y aparecen solo sus equipos; **cada equipo es su propia ficha**. Puedes buscar por nombre, SKU o descripción y ordenar por las columnas. Al abrir una ficha ves todos sus datos:
  - Nombre y Código (SKU)
...
1. Entra a **Productos** y abre la **carpeta de la categoría** donde va el equipo (por ejemplo `congeladores`).
2. Ya dentro de la carpeta, haz clic en el botón para agregar una entrada nueva. **Es importante crearlo dentro de la carpeta**: ahí es donde el producto toma su categoría.
3. Llena al menos **Nombre** y **Código (SKU)**.
4. Sube las fotos en **Fotos del producto** (se guardan solas).
